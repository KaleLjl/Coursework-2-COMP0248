...
class DepthTableDataset(Sun3D):
    """
    Sun3D depth dataset for point cloud segmentation.
    """
    def __init__(self, data_root, sequences, split='train', num_points=1024,
                 transform=None):
        """
    ...
    def __getitem__(self, idx):
        ...
        # Generate point cloud with depth
        ...
        # Now apply transforms after creating the point cloud
        if self.transform:
        ...
        for i, (x, y, z) in enumerate(curr_points[:, :3]):
            if z > 0:
                ...
                # Check if the projected point is inside the image and valid
        ...
        for key in pc_sample:
            if isinstance(pc_sample[key], np.ndarray):
                pc_sample[key] = torch.from_numpy(
                    pc_sample[key].astype(np.float32))
